           ...
           <div id="html-view-validations">
             {{-- <form class="formValidate0" id="formValidate0" method="post" action="{{route('patner-create')}}" enctype="multipart/form-data"> --}} 
                ...
                @if(isset($response->id)) 
                    <form class="formValidate0" id="formValidate0" enctype="multipart/form-data" method="post" action="{{route('industry.update',[$response->id])}}">
                      {{ method_field('PUT') }}
                    <input type="hidden" name="id" value="{{$response->id }}">
                    
                @else 
                    <form class="formValidate0" id="formValidate0" method="post" action="{{route('industry.store')}}" enctype="multipart/form-data"> 
                @endif
