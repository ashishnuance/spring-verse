@if(isset($user_personal_detail) && !empty($user_personal_detail))
             
@foreach($user_personal_detail as $key => $val_user)
   ...
   <div class="thumbnail">
      <div class="thumb-img">
         <a href="{{route('profile',$val_user->username)}}">
            @if(isset($val_user->provider_id) && $val_user->provider_id!='' && $val_user->social_login!='' && $val_user->profile_image!='')
               <img src="{{ $val_user->profile_image }}" alt="..." onerror="this.onerror=null;this.src='{{default_image($val_user->profile_image)}}';" class="profile-image"/>
            @else
            <img src="{{ (isset($val_user->profile_image)) ? default_image($val_user->profile_image) : default_image()}}" alt="..." class="profile-image"/>
            {{-- <img src="{{ empty($val_user->profile_image) ? url('/public/').'/frontend/images'.((isset($front_default_path) && $front_default_path!='') ? $front_default_path : '') : url('/public/').'/uploads/profile/'.$val_user->profile_image }}" alt="..."> --}}
            @endif
         </a>
         <div class="add-cart">
            <ul>
               <li>
                  <a href="javascript:void:;" class="like_profile_btn" data-profileid="{{$val_user->id}}">
      ...
      <div class="caption">
         <div class="caption-details">
            ...
            {{-- <p class="member-role">{{ (isset($val_user->user_meta->member_role) && !empty($val_user->user_meta->member_role )) ? $val_user->user_meta->member_role :  '' }}</p> --}}
            ...
            <h5 class="main-locate" ><img src="{{asset('frontend/images/icon-location.png')}}" class="icon-location" alt="" style="margin-right: 5px;">{{ (isset($val_user->user_meta->address) && $val_user->user_meta->address!='') ? $val_user->user_meta->address :  '' }}</h5>
            @endif
         </div>
